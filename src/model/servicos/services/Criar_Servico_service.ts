...
export class CriarServicoService {
    ...
    async execute(data: CriarServicoDTO) {
        ...
        if(data.metragem_usada > material.quantidade) throw new BadRequestError("Quantidade de materiais insuficiente");
        
        const carro = await this.carroRepository.BuscarPorId(data.carroId);
        if (!carro) throw new NotFoundError("Carro nao encontrado");
        if (carro.clienteId !== data.clienteId) throw new BadRequestError("Carro não pertence ao cliente");
